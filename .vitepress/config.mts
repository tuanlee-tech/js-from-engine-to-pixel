...
export default withPwa(defineConfig({
  title: 'JavaScript: From Engine to Pixel',
  description: 'Hiểu JavaScript từ engine internals đến rendering pipeline và system design — không học syntax, học cơ chế.',
  lang: 'vi-VN',
  base: base,
  head: [
    ...
    ['meta', { property: 'og:title', content: 'JavaScript: From Engine to Pixel' }],
    ['meta', { property: 'og:description', content: 'Hiểu JavaScript từ engine internals đến rendering pipeline và system design — không học syntax, học cơ chế.' }],
  ...
  themeConfig: {
    ...
    socialLinks: [
      { icon: 'github', link: 'https://github.com/tuanlee-tech/js-from-engine-to-pixel' }
    ],
  ...
  vue: {
    template: {
      compilerOptions: {
        isCustomElement: (tag) => {
          return tag.toLowerCase().indexOf('-') !== -1 || [
            'script', 'style', 'input', 'img', 'object', 'svg', 'a', 'path'
  ...
  pwa: {
    ...
    manifest: {
      name: 'JavaScript: From Engine to Pixel',
      short_name: 'JS Engine→Pixel',
      description: 'Hiểu JavaScript từ engine internals đến rendering pipeline và system design.',
      theme_color: '#F0DB4F',
